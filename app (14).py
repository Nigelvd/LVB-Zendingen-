...
with tab2:
    ...
    if hermeting_bestand and bol_verzendingen:
        ...
        # Automatisch juiste rij zoeken voor kolomnamen in verzendexport
        for i in range(5):
            df_verzonden_tmp = pd.read_excel(bol_verzendingen, header=i)
            if 'EAN' in df_verzonden_tmp.columns:
                df_verzonden = df_verzonden_tmp.copy()
                break
        else:
            st.error("❌ Kolom 'EAN' niet gevonden in Bol verzendexport. Controleer het bestand.")
            st.stop()
