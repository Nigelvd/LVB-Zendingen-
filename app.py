...
with tab1:
    ...
    if bol_file and fulfilment_file:
        ...
        if gebruik_14_dagen and bol_14_file:
            ...
            st.error("❌ Kan het 14-dagen Excel-bestand niet correct verwerken. Details: " + str(e))
            st.stop()
        except Exception as e:
            st.error("❌ Kan het 14-dagen bestand niet correct lezen. Zorg dat kolom A (EAN) en kolom I (verkopen) aanwezig zijn.")
            st.stop()
            df_14["EAN"] = df_14["EAN"].astype(str)
            df_14["Verkopen_14"] = pd.to_numeric(df_14["Verkopen_14"], errors="coerce").fillna(0).astype(int)
