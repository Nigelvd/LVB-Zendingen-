...
with tab2:
    ...
    if hermeting_bestand and bol_verzendingen:
        ...
        # Lees verzendbestand zonder headers en wijs handmatig kolommen toe
        df_verzonden_raw = pd.read_excel(bol_verzendingen, header=None)
        df_verzonden = pd.DataFrame()
        df_verzonden['EAN'] = df_verzonden_raw.iloc[:, 2]
        df_verzonden['Ordernummer'] = df_verzonden_raw.iloc[:, 5]
        df_verzonden['Verzonden formaat'] = df_verzonden_raw.iloc[:, 7]
        if 'EAN' in df_verzonden.columns:
            df_verzonden.dropna(subset=['EAN'], inplace=True)
            df_verzonden['EAN'] = df_verzonden['EAN'].astype(str)
        else:
            st.error("❌ Kolom 'EAN' kon niet worden aangemaakt. Controleer of kolom C in het bestand correct is.")
            st.stop()
