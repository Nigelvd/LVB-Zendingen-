        ...
        if not eigen_link or any(not link for link in concurrent_links):
            ...
            if st.button("📈 Vergelijk Listings"):
                if not eigen_link or any(not link for link in concurrent_links):
                    ...
                    st.success("Links succesvol ontvangen! Resultaten van vergelijking hieronder:")

                    # Simuleer de GAP-analyse
                    import pandas as pd
                    import random

                    def generate_mock_gap_analysis(own_title):
                        data = {
                            "Element": [
                                "Titel lengte (tekens)",
                                "Prijs (€)",
                                ...
                                "Levertijd (dagen)",
                                "Aantal USP bullet points",
                                "Lengte productomschrijving (woorden)",
                                "Aantal afbeeldingen",
                                "Merkconsistentie (ja/nee)",
                                "Voorraadstatus"
                            ],
                            "Eigen product": [
                                len(own_title),
                            ...
                            "Concurrent 1": [
                                ...
                                random.choice(["Op voorraad", "Niet op voorraad"])
                            ],
                            "Concurrent 2": [
                                random.randint(30, 80),
                                round(random.uniform(17, 25), 2),
                    ...
                    df = generate_mock_gap_analysis("Voorbeeld producttitel met zoekwoorden")
