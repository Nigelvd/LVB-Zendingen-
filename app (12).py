        ...
        if vorige_zending_file and filter_oude_producten:
            try:
                ...
                if not df_uitgefilterd.empty:
                    ...
                    st.download_button("📥 Download gefilterde producten als Excel", data=buffer.getvalue(), file_name="Uitgefilterde_Producten.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

                    csv = df_uitgefilterd.to_csv(index=False).encode('utf-8')
                    st.download_button("📄 Download gefilterde producten als CSV", data=csv, file_name="Uitgefilterde_Producten.csv", mime="text/csv")
